...
function patchWebpackConfig(config: webpack.Configuration, options: Options) {
  ...
  delete (<any>config.entry).polyfills;
  delete (<any>config.entry)['polyfills-es5'];
  delete (<any>config.optimization).runtimeChunk;
  delete (<any>config.optimization).splitChunks;
  delete (<any>config.entry).styles;
  ...
  if (sharedLibs) {
    ...
    sharedLibsArr.forEach(sharedLibName => {
      ...
      (<any>config.externals)[0][sharedLibName] = sharedLibName; // define external for code
      (<any>config.externals).push((_context: any, request: any, callback: any) => {
  ...
  if (ngCompilerPluginInstance) {
    (<any>ngCompilerPluginInstance)._entryModule = options.modulePath;
  }

  // preserve path to entry point
  // so that we can clear use it within `run` method to clear that file
  if (config.entry) {
    entryPointPath = (<any>config.entry).main[0];
  }
